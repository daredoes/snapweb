...
const StreamSlider: React.FC<StreamSliderProps> = ({ streamId: id }) => {
  ...
  const handleChangeCommitted = useCallback((e: Event | React.SyntheticEvent<Element, Event>, v: number | number[]) => {
    api.streamControlSetPosition({id: id, params: {position: v as number}})
  ...
  return (
    <>
      <Slider onChangeCommitted={handleChangeCommitted} disabled={!stream.properties.canSeek} size={'small'} color={'secondary'} min={0} onChange={handleChange} value={internalPlaytime || totalPlaytime} max={stream.properties.metadata?.duration || 2} />
