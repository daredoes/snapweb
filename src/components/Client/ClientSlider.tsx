...
import { apiAtom, clientsAtom, connectedAtom, streamsAtom } from "src/atoms/snapclient"
...
const ClientSlider: React.FC<ClientSliderProps> = ({ clientId: id, ...props }) => {
  ...
  const handleChangeCommitted = useCallback((e: Event | React.SyntheticEvent<Element, Event>, v: number | number[]) => {
    api.clientSetVolume({id: id, volume: {
      percent: v as number,
